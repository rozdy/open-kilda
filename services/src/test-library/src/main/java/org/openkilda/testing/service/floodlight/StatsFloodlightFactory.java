...
public class StatsFloodlightFactory implements MultiFloodlightFactory {
    @Autowired
    private ApplicationContext context;

    @Value("#{'${floodlight.regions}'.split(',')}")
    private List<String> regions;
    @Value("#{'${floodlight.controllers.stat.endpoints}'.split(',')}")
    private List<String> statsControllers;
    ...
    public FloodlightService getFloodlightService(String region) {
        String endpoint = statsControllers.get(getRegionIndex(region));
